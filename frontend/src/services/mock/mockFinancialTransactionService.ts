...
import { resolveQuantityCalculation, type FinancialCalculationMode, type FinancialQuantityUnit } from '../../finance/quantityCalculator';
...
const toDto = (input: CreateFinancialTransactionInput, id: string): FinancialTransaction => {
  ...
  const quantity = resolveQuantityCalculation({
    mode: (input.calculationMode as FinancialCalculationMode) || 'total_only',
    quantity: input.quantity,
    quantityUnit: input.quantityUnit as FinancialQuantityUnit | undefined,
  ...
  return {
    ...
    amountIsCalculated: quantity.amountIsCalculated,
    unitPriceIsCalculated: quantity.unitPriceIsCalculated,
