...
export const geospatialService = {
  ...
  requestHistoryBackfill: async (fieldId: string): Promise<boolean> => {
    try {
      ...
      return geospatialService.refreshIntelligence(fieldId);
    }
  },
  ...
  getFieldWeather: async (fieldId: string): Promise<FieldWeather | null> => {
    try {
      ...
      await EntityCache.setOne<FieldWeather>(WEATHER_CACHE, fieldId, response.data);
      return response.data;
    } catch {
      return EntityCache.getOne<FieldWeather>(WEATHER_CACHE, fieldId);
    }
  },

  getSpatialProfile: async (fieldId: string): Promise<FieldSpatialProfile | null> => {
    try {
      const response = await api.get<FieldSpatialProfile>(`/api/v1/fields/${fieldId}/spatial-profile`);
      await EntityCache.setOne<FieldSpatialProfile>(PROFILE_CACHE, fieldId, response.data);
      return response.data;
    } catch {
      return EntityCache.getOne<FieldSpatialProfile>(PROFILE_CACHE, fieldId);
    }
  },

  getIntelligence: async (fieldId: string): Promise<FieldIntelligenceSummary | null> => {
    try {
      const response = await api.get<FieldIntelligenceSummary>(`/api/v1/fields/${fieldId}/intelligence`);
      await EntityCache.setOne<FieldIntelligenceSummary>(INTELLIGENCE_CACHE, fieldId, response.data);
      return response.data;
    } catch {
      return EntityCache.getOne<FieldIntelligenceSummary>(INTELLIGENCE_CACHE, fieldId);
    }
  },
  ...
  getSatelliteDates: async (fieldId: string): Promise<SatelliteDate[]> => {
    try {
      ...
      await EntityCache.setOne<SatelliteDate[]>(SATELLITE_DATES_CACHE, fieldId, response.data);
      return response.data;
    } catch {
      return (await EntityCache.getOne<SatelliteDate[]>(SATELLITE_DATES_CACHE, fieldId)) ?? [];
    }
  },

  /**
   * Resolves drawable overlays for a field. Cached per layer and date so a grower
   * who already opened a layer still sees it after losing signal.
   */
  getMapData: async (
    fieldId: string,
    layerIds: string[],
    observationId?: string
  ): Promise<FieldMapData | null> => {
    const cacheKey = `${fieldId}:${layerIds.join(',')}:${observationId ?? 'latest'}`;
    try {
  ...
  refreshSatellite: async (fieldId: string): Promise<boolean> => {
    try {
      await api.post(`/api/v1/fields/${fieldId}/satellite/refresh`);
      return true;
    } catch {
      return false;
    }
  ...
  refreshIntelligence: async (fieldId: string): Promise<boolean> => {
    try {
  ...
  getMapLayers: async (): Promise<MapLayerCatalog | null> => {
    try {
      const response = await api.get<MapLayerCatalog>('/api/v1/map/layers');
      await EntityCache.setOne<MapLayerCatalog>(LAYERS_CACHE, 'catalog', response.data);
      return response.data;
    } catch {
      return EntityCache.getOne<MapLayerCatalog>(LAYERS_CACHE, 'catalog');
    }
  },
};
