...
const aggregateWeather = (snapshots: DailyWeatherSnapshot[], range: HistoryRange): WeatherPoint[] => {
  ...
  const useWeeks = range === '90d';

  for (const day of sorted) {
    const date = toDate(day.date);
    const key = useWeeks ? format(date, 'yyyy-ww') : format(date, 'yyyy-MM');
  ...
  return [...months.values()].map((bucket) => ({
    label: format(bucket.date, useWeeks ? 'd MMM' : 'MMM yy'),
...
const FieldHistoryScreen = () => {
  ...
  const vegetationPoints = useMemo<VegetationPoint[]>(() => {
    ...
    return observations
      ...
      .map((item) => ({
        label: format(toDate(item.observationDate), range === '90d' ? 'd MMM' : 'MMM yy'),
  ...
  return (
    <ScreenLayout scroll contentContainerStyle={styles.content}>
      ...
      {gathering ? (
        <View style={[styles.gathering, { backgroundColor: colors.successLight }]}>
